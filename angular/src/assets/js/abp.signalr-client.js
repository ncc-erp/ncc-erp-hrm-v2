...
(function () {
    ...
    function configureConnection(connection) {
            ...
            if (tries <= abp.signalr.maxTries) {
                connection.start()
                    .then(function () {
                        ...
                        tries = 1;
                        console.log('Reconnected to SignalR server!', connection);
        ...
        connection.on('getNotification', function (notification) {
            ...
            abp.event.trigger('abp.signalr.received', notification);
        });
    }
    ...
    function connect() {
        ...
        startConnection(url, configureConnection)
            .then(function (connection) {
                console.log('Connected to SignalR server!', url); //TODO: Remove log
                abp.event.trigger('abp.signalr.connected');
                // Call the Register method on the hub
                connection.invoke('register').then(function () {
                    console.log('Registered to the SignalR server!' + url); //TODO: Remove log
                });
            })
