...
import { ChartDto, ChartSelectionDto, CreateChartDto, UpdateChartDto } from "@app/service/model/chart-settings/chart.dto";
  ...
  implements OnInit
{
  ...
  getAllFilterData() {
    const selectionData: ChartSelectionDto =
      this.chartService.selectionData;
  ...
  saveAndClose() {
    ...
    if (this.formGroupChart.value.name && this.formGroupChart.value.name.length !== 0) {
      if (this.isEdit) {
        this.subscription.push(
          this.chartService
            .update(updateChart)
