...
@Component({
  selector: "app-date-selector-homepage",
  templateUrl: "./date-selector-homepage.component.html",
  styleUrls: ["./date-selector-homepage.component.css"],
})
export class DateSelectorHomepageComponent
  ...
  implements OnInit, OnChanges
{
  ...
  dateText: string;
  isBtnPrev: boolean;
  DATE_TIME_OPTIONS_HOME = DATE_TIME_OPTIONS_HOME;
  currentDate = moment();
  ...
  ngOnChanges(change: SimpleChanges) {
    ...
    if (change["defaultDateFilter"]) {
      this.dateText = this.getDisplayText(
        this.defaultDateFilter.fromDate,
        this.defaultDateFilter.toDate
      );
    }
  }
  getDateOptions() {
  ...
  handleSelectDay() {
    if (this.searchDay) {
      this.onDateSelectorChange.emit({
        ...
        dateText: moment(this.searchDay).format(DateFormat.MM_YYYY),
      });
    }
  }
  ...
  private getDisplayText(
    ...
    return `${fromDate.format(DateFormat.MM_YYYY)}${spaceFormat}${toDate.format(
      DateFormat.MM_YYYY
    )}`;
  }
}
